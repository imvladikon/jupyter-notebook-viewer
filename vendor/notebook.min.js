...
(function () {
  ...
  const map_chars = {
    ...
    escape_markdown = (string) => string.replace(/[\|\*\(\)\[\]\+\-\\_`#<>]/g, m => map_chars[m]);
    nb.Cell.prototype.renderers = {
        markdown: function () {
            var el = makeElement("div", [ "cell", "markdown-cell" ]);
          text = joinText(this.raw.source).replace(/\$.*?\$/gm, (match, $1) => {
            return  escape_markdown(match)
            // console.log($1)
          });

          el.innerHTML = nb.markdown(text);
